...
class DbManager(var client: PgPool) {
     ...
     * 書籍情報取得処理
     *
     * @param type 検索条件 1:条件なし、2:著者指定、3:書籍ID指定
     * @param authorName 著者
     * @param id 書籍ID
     *
     * @return 書籍情報
     */
    fun select(type: Int, authorName: String?, id: Int?): List<BookInfo> {
        return when (type) {
            1 -> client.rxPreparedQuery("SELECT ROW_NUMBER() OVER () AS no, * FROM book_info")
            2 -> client.rxPreparedQuery("SELECT ROW_NUMBER() OVER () AS no, * FROM book_info WHERE author_name LIKE $1", Tuple.of("$authorName%"))
